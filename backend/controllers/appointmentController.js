...
export const getAppointments = async (req, res) => {
    ...
    const items = await Appointment.find(filter)
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit).populate("doctorId", "name specialization owner imageUrl image").lean();
...
export const getAppointmentsByPatient = async (req, res) => {
    try {
        ...
        const resolveCreateBy = queryCreateBy || clerkUserId || null;

        console.log('resolveCreateBy (query or req.auth.userId):', resolveCreateBy);

        if (!resolveCreateBy && !req.query.mobile) {
        ...
        const filter = {};
        if (resolveCreateBy) { filter.createdBy = resolveCreateBy; }
